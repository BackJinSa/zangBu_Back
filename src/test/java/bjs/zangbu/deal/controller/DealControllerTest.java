 ...
 * -------------------------------------------------------------
 * • Spring-test 의 MockMvc stand-alone 모드만 사용
 * • 케이스
 * 1) 매매(TRADING)  → /contracts/sale_contract.pdf
 * 2) 월세(MONTHLY) → /contracts/lease_contract.pdf
 */
class DealControllerTest {
    ...
    private static class StubDealMapper implements DealMapper {
        ...
        @Override
        public List<DealWithChatRoom> getWaitingDealsWithChatRoom(String userId) {
            return Collections.emptyList();
        }

        @Override
        public List<Deal> getAllDealByUserId(String userId) {
            return Collections.emptyList();
        }
    }
