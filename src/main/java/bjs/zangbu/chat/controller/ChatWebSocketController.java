...
public class ChatWebSocketController {
    ...
    public void sendMessage(
            @DestinationVariable String chatRoomId, @Payload ChatRequest.SendMessageRequest request) {

        ChatResponse.SendMessageResponse response = chatService.sendMessage(chatRoomId, request);   //메시지 db에 저장
        messagingTemplate.convertAndSend("/topic/chat/room/" + chatRoomId, response);  // 채팅방 구독자에게 메시지 전송
    }
