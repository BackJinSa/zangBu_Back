 ...
 * 로그인, 로그아웃, 아이디(이메일) 찾기, 비밀번호 재설정, 본인인증, 회원가입, 이메일/닉네임 중복 확인, 토큰 재발급 기능을
 * 제공합니다.
 */
@Log4j2
@RestController
...
public class AuthController {
  ...
  public ResponseEntity<?> getMyInfo(@AuthenticationPrincipal CustomUser customUser) {
    ...
    return ResponseEntity.ok(Map.of(
        "name", customUser.getMember().getName(),
        "email", customUser.getUsername(),
        "phone", customUser.getMember().getPhone()));
  }

  /**
   * 로그아웃 요청 처리.
   * {@code POST /auth/logout} 엔드포인트를 통해 현재 로그인된 사용자를 로그아웃 처리하고, Refresh Token 쿠키를
   * 삭제합니다.
   *
   * @param customUser 인증된 사용자 정보 (Spring Security Principal)
   * @param response   HTTP 응답 객체 (Refresh Token 쿠키 삭제를 위해 사용)
   * @return 로그아웃 성공 시 200 OK 응답, 실패 시 400 Bad Request 또는 500 Internal Server
   *         Error 응답
   */
  @ApiOperation(value = "로그아웃", notes = "로그아웃 성공 시 토큰을 삭제합니다.")
  @ApiResponses({
   ...
   * @return 이메일 찾기 성공 시 {@link EmailAuthResponse}와 함께 200 OK 응답, 실패 시 400 Bad
   *         Request 또는 500 Internal Server Error 응답
   */
  @ApiOperation(value = "이메일 찾기", notes = "이름과 휴대폰 번호 기반으로 이메일을 찾습니다.", response = EmailAuthResponse.class)
  @ApiResponses({
  ...
  public ResponseEntity<?> findEmail(
      @ApiParam(value = "이메일 찾기 요청 DTO (이름, 전화번호 입력)", required = true) @RequestBody EmailAuthRequest request,
      @ApiIgnore @AuthenticationPrincipal CustomUser customUser) {
   ...
   * 인증 상태 세션에 저장
   */
  @ApiOperation(value = "본인인증 요청", notes = "본인인증 수행 후 성공 시 세션에 인증 상태를 저장합니다.")
  @ApiResponses({
  ...
  public ResponseEntity<?> verifyAuthenticity(
      @ApiIgnore @AuthenticationPrincipal CustomUser customUser,
      @ApiParam(value = "본인인증 성공한 데이터 DTO", required = true) @RequestBody VerifyCodefRequest request,
      HttpSession session) {
  ...
  @PostMapping(value = "/verify/password", consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
  public ResponseEntity<?> verifyAuthenticityForPassword(
    ...
    try {
      ...
      // (이 로직은 아래 3) 서비스에 구현)
      AuthResponse.PasswordVerifyResponse resp = authService.verifyPasswordFlow(sessionId);
   ...
   */
  @ApiOperation(value = "본인인증 CODEF 진위인증 사용", notes = "CODEF를 이용한 진위인증을 처리합니다. (현재 임시 구현)")
   ...
   * @return 회원가입 성공 시 200 OK 응답, 실패 시 409 Conflict (중복) 또는 500 Internal Server
   *         Error 응답
   */
  @ApiOperation(value = "회원가입", notes = "이메일과 닉네임 중복 확인 후 회원가입할 수 있게 합니다.")
  @ApiResponses({
  ...
  public ResponseEntity<?> signUp(
      @ApiParam(value = "회원가입 요청 DTO", required = true) @RequestBody SignUp request) {
    try {
   ...
   */
  @ApiOperation(value = "이메일 중복 확인", notes = "입력한 이메일이 사용 중인지 여부를 확인합니다.")
  @ApiResponses({
  ...
  public ResponseEntity<?> checkEmail(
      @ApiParam(value = "이메일", example = "example@zangbu.com", required = true) @RequestBody AuthRequest.EmailCheck request) {
   ...
   */
  @ApiOperation(value = "닉네임 중복 확인", notes = "입력한 닉네임이 사용 중인지 여부를 확인합니다.")
  @ApiResponses({
  ...
  public ResponseEntity<?> checkNickname(
      @ApiParam(value = "닉네임", example = "김철수123", required = true) @RequestBody AuthRequest.NicknameCheck request) {
  ...
  /**
   * 토큰 재발급 요청 처리.
   * {@code POST /auth/reissue} 엔드포인트를 통해 Access Token이 만료되었을 때 Refresh Token을
   * 사용하여 새로운 Access Token과 Refresh Token을 발급합니다.
   ...
   * @return 토큰 재발급 성공 시 {@link TokenResponse}와 함께 200 OK 응답, 실패 시 400 Bad Request
   *         (토큰 만료/유효하지 않음) 또는 409 Conflict (토큰 없음) 응답
   */
  @ApiOperation(value = "토큰 재발급", notes = "refresh 토큰이 유효한 경우, 새로운 access 토큰을 발급합니다.", response = TokenResponse.class)
  @ApiResponses({
